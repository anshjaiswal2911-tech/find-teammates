...
export function Match() {
    ...
    useEffect(() => {
        const loadProfiles = async () => {
            const realProfiles = await dbService.getAllProfiles();
            // Filter out the current user
            const otherProfiles = realProfiles.filter(p => p.id !== currentUser.id);
            
            // If we have real profiles, use them. Otherwise fallback to mock for demo.
            const sourceUsers = otherProfiles.length > 0 ? otherProfiles : []; 
            const queue = generateMatches(currentUser, sourceUsers);
            setMatchQueue(queue);
        };
    ...
    const handleRestart = async () => {
        const realProfiles = await dbService.getAllProfiles();
        const otherProfiles = realProfiles.filter(p => p.id !== currentUser.id);
        const reshuffled = generateMatches(currentUser, otherProfiles);
                                            ...
                                            >
                                                <div className="relative">
                                                    <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-purple-300">
                                                        <img
                                                            src={m.user.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(m.user.name || 'User')}&background=7C3AED&color=fff`}
                                                            alt={m.user.name || 'User'}
                                                            className="w-full h-full object-cover"
                                                            referrerPolicy="no-referrer"
                                                            onError={e => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(m.user.name || 'User')}&background=7C3AED&color=fff`; }}
                                                        />
                                                    </div>
                                                ...
                                                <p className="text-[10px] font-black text-gray-700">{(m.user.name || 'User').split(' ')[0]}</p>
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <p className="text-white/70 text-sm font-medium leading-relaxed">
                                        {currentMatch
                                            ? <>Analyzing <span className="text-white font-bold">{currentMatch.user.name || 'User'}</span> for you in real-time.</>
