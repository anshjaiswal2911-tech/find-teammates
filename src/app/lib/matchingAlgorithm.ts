...
export function getSkillOverlap(a: User, b: User): string[] {
    ...
    return aSkills.filter(s => bSkills.includes(s));
}
...
export function getComplementarySkills(a: User, b: User): string[] {
    const aSkills = a.skills || [];
    const bSkills = b.skills || [];
    return bSkills.filter(s => !aSkills.includes(s)).slice(0, 4);
}
...
export function getInterestOverlap(a: User, b: User): string[] {
    ...
    return aInterests.filter(i => bInterests.includes(i));
}
...
function experienceScore(a: User, b: User): number {
    ...
    const aLevel = levels[a.experience] || 1;
    const bLevel = levels[b.experience] || 1;
...
function buildExplanation(
    ...
    const bName = b.name || 'User';
    if (score >= 80) {
        return `You and ${bName} are a stellar team match! You share ${skillOverlap.length} core skills and ${bName} brings ${complementary.slice(0, 2).join(' & ')} to complement your stack.`;
    }
    if (score >= 60) {
        return `Strong match! ${bName} shares your passion for ${getInterestOverlap(a, b)[0] || 'building products'} and your combined skills cover the full stack.`;
    }
    if (score >= 40) {
        return `${bName} has a different but complementary skillset — together you'd fill important gaps on any team.`;
    }
    return `${bName} could be a wild card match — different background, fresh perspective, and new skills to learn from each other.`;
}
